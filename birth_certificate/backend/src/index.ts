...
app.use((req, res, next) => {
  ...
  console.log(JSON.stringify({ level: 'info', service: 'birth-certificate', requestId, method: req.method, path: req.path, event: 'request.received' }));
  next();
});
...
app.post('/submit-form', async (req, res) => {
  ...
  const validationErrors = validateBirthForm(formData);
...
const start = async () => {
  try {
    ...
    app.listen(port, () => {
      console.log(`Birth Certificate Service listening on port ${port}`);
    });
  } catch (error) {
